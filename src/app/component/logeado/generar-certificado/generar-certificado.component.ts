...
export class GenerarCertificadoComponent implements OnInit {
         ...
         this.filteredPeriod = data.filter(function(car) {
          ...
          datosEnero = this.filteredPeriod.filter(function(car) {
            return car.periodo === 'ENERO'? car.periodo = 'ENERO - FEBRERO': ''
                || car.periodo === 'MAYO'? car.periodo = 'MAYO - JUNIO': '' || car.periodo === 'MARZO'? car.periodo = 'MARZO - ABRIL': '' || car.periodo === 'JULIO'? car.periodo = 'JULIO - AGOSTO': ''
                || car.periodo === 'SEPTIEMBRE'? car.periodo = 'SEPTIEMBRE - OCTUBRE': '' || car.periodo === 'NOVIEMBRE'? car.periodo = 'NOVIEMBRE - DICIEMBRE': ''
          });
        ...
        (data) => {
          ...
          if (this.seleccionPeriodicidad === 1) {
            ...
            if (this.datos.periodOne === 'ENERO' ) {
              ...
              this.fechaUno.split('').reverse().join('');
              this.fechaDos = `/12/${this.fechaPeriodoDos.fecha.slice(0, -16)}`;
          ...
          this.descarga = true;
              ...
              if (this.seleccionPeriodicidad === 1) {
                ...
                if (this.datos.periodOne === 'ENERO' ) {
                  ...
                  this.fechaUno.split('').reverse().join('');
                  this.fechaDos = `/12/${this.fechaPeriodoDos.fecha.slice(0, -17)}`;
                } else if( this.datos.periodOne === 'SEPTIEMBRE') {
  ...
  downloadPDFIca(){
    ...
    var fecha =  `${this.fecha.getDate()}/${this.fecha.getMonth() + 1  }/${this.fecha.getFullYear()}`;
    ...
    doc.text(72, 45, 'CALLE 119 No. 7 - 75 TELEFONO 6030303');
    ...
    doc.text(130, 105, 'PORCENTAJE x MIL');
    ...
    doc.setFontSize(10);
      ...
      for (let index = 0; index < this.listaDatosAnual.length; index++) {
        ...
        var retenidoTexto = trans.NumerosALetras(retenido);
        ...
        for (let index = 0; index < this.listaDatosAnual.length; index++) {
          ...
          doc.text(115, 110 + data, new Intl.NumberFormat("de-DE").format(element.base.toString()));
    ...
    doc.setFontType('bold');
    doc.setFontSize(10);
    doc.text(20, 155 + data, 'FUNDACION SANTA FE DE BOGOTA');
    doc.text(20, 160 + data, 'NIT: 860037950-2');
    doc.text(20, 165 + data, `FECHA DE EXPEDICION:  ${fecha.toString()}`);

    doc.setFont('helvetica');
    doc.setFontType('normal');
    doc.setFontSize(7);
    doc.text(20, 175 + data, 'NOTA: LAS PERSONAS JURIDICAS PODRAN ENTREGAR LOS CERTIFICADOS DE RETENCION EN LA FUENTE, EN FORMA CONTINUA IMPRESA');
    ...
    doc.text(20, 190 + data, 'DEL AGENTE DE RETENCIÓN O EN LA SEDE DEL RETENIDO (CONCEPTO DIAN 105489 DE 24-12-2007).LA UTILIZACION DE ESTE CERTIFICADO');
  ...
  downloadPDFRenta(){
    ...
    doc.setFontSize(10);
      ...
      for (let index = 0; index < this.listaDatosAnual.length; index++) {
        ...
        doc.text(113, 110 + data, new Intl.NumberFormat("de-DE").format(element.base.toString()));
    ...
    doc.setFontType('bold');
    doc.setFontSize(10);
    doc.text(20, 170 + data, 'FUNDACION SANTA FE DE BOGOTA');
    doc.text(20, 175 + data, 'NIT: 860037950-2');
    doc.text(20, 180 + data, `FECHA DE EXPEDICION:  ${fecha.toString()}`);

    doc.setFont('helvetica');
    doc.setFontType('normal');
    doc.setFontSize(7);
    doc.text(20, 190 + data, 'NOTA: LAS PERSONAS JURIDICAS PODRAN ENTREGAR LOS CERTIFICADOS DE RETENCION EN LA FUENTE, EN FORMA CONTINUA IMPRESA');
    ...
    doc.text(20, 205 + data, 'DEL AGENTE DE RETENCIÓN O EN LA SEDE DEL RETENIDO (CONCEPTO DIAN 105489 DE 24-12-2007).LA UTILIZACION DE ESTE CERTIFICADO');
